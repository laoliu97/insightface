...
parser.add_argument("--log_dir", type=str, default="./logs/insightface/arcface", required=True)
...
parser.add_argument('--train_batches', type=int, default=400)
...
def extract_info_from_file(log_file, result_dict, speed_dict):
    ...
    with open(log_file) as f:
        ...
        speeds = list()
        for line in lines:
            if "Training:" in line and "samples/sec " in line:
                print(line)
                p1 = re.compile(r'-Speed (\d+\.\d+) samples/sec', re.S)
                s = re.findall(p1, line)
                speed = round(float(s[0].strip()), 1)
                speed = speed * node_num
                speeds.append(speed)
...
if __name__ == "__main__":
    assert args.warmup_batches % 20 == 0 and args.train_batches > args.warmup_batches
    extract_result()
